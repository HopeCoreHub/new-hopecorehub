-- Add likes to posts using anonymous users (NULL user_id is allowed for anonymous interactions)
-- Since the RLS policies allow anyone to view likes, we can create anonymous likes

-- First, let's add some likes with NULL user_id for anonymous likes
INSERT INTO public.forum_post_likes (post_id, user_id)
SELECT 
  p.id as post_id,
  NULL as user_id  -- Anonymous likes
FROM public.forum_posts p
WHERE random() < 0.6  -- About 60% of posts get anonymous likes
ON CONFLICT DO NOTHING;

-- Add some more likes using the existing user_ids from the posts themselves
INSERT INTO public.forum_post_likes (post_id, user_id)
SELECT DISTINCT
  p1.id as post_id,
...
WHERE p1.id != p2.id 
  ...
  AND random() < 0.4  -- About 40% chance for each combination
ON CONFLICT DO NOTHING;

-- Add replies to posts using anonymous replies
INSERT INTO public.forum_replies (post_id, user_id, content, is_anonymous, created_at)
SELECT 
  p.id as post_id,
  NULL as user_id,  -- Anonymous replies
  CASE 
  ...
  p.created_at + (random() * interval '6 hours') as created_at  -- Random time after post creation
FROM public.forum_posts p
WHERE random() < 0.8  -- 80% of posts get at least one reply
ORDER BY p.created_at;

-- Add some additional replies to make conversations more active
INSERT INTO public.forum_replies (post_id, user_id, content, is_anonymous, created_at)
SELECT 
  p.id as post_id,
  NULL as user_id,
...
FROM public.forum_posts p
WHERE random() < 0.5  -- 50% chance for additional replies
ORDER BY p.created_at;